...
Definition pairZ (x y : Z) : prodZZ := existT (fun _ : Z => Z) x y.
(** Lexicographic of pair of elements of Z *)
 
Definition lexZ := lexprod Z (fun _ : Z => Z) (Zwf 0) (fun _ : Z => Zwf 0).
(* Prime for Z *)
 
Definition Zprime (z : Z) := (0 < z)%Z /\ prime (Z.abs_nat z).
(* Odd for Z *)
 
Definition Zodd (z : Z) := exists x : Z, z = (2 * x + 1)%Z.
(* Division *)
 
Definition Zdivides (z1 z2 : Z) := divides (Z.abs_nat z1) (Z.abs_nat z2).
(* Square root *)
 
Definition sqr (z : Z) := Z_of_nat (sqr (Z.abs_nat z)).
(* One *)
...
Definition mod_ (z1 z2 : Z) :=
  Z_of_nat (snd (pdiv (Z.abs_nat z1) (Z.abs_nat z2))).
...
apply Zle_not_lt; apply Z.le_trans with (p * q)%Z; auto with zarith.
Qed.
...
intros H3; Contradict H2; auto with zarith.
apply Zlt_not_le; apply Z.le_lt_trans with (q * p)%Z; auto with zarith.
...
Theorem inj_abs : forall x : Z, (0 <= x)%Z -> Z_of_nat (Z.abs_nat x) = x.
